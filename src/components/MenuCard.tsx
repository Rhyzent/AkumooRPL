...
export const MenuCard = ({ item }: { item: MenuItem }) => {
  ...
  const pct = hasDiscount ? Math.round(((item.price - item.discountPrice!) / item.price) * 100) : 0;
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-3xl border border-border/60 bg-card shadow-card transition-all hover:-translate-y-1 hover:shadow-soft">
      <div className="relative aspect-[5/4] overflow-hidden bg-muted">
        ...
        {hasDiscount && (
          <span className="absolute right-4 top-4 rounded-full bg-destructive px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-destructive-foreground shadow-card">
            -{pct}%
          </span>
        )}
      </div>
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <div className="mt-auto flex items-center justify-between pt-3">
          <div className="flex flex-col leading-tight">
            {hasDiscount && (
              <span className="text-xs text-muted-foreground line-through">{formatRp(item.price)}</span>
            )}
            <span className="font-display text-2xl font-semibold">{formatRp(finalPrice)}</span>
          </div>
